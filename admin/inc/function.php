...
function edit_lang(){
		...
		if(isset($_GET['edit_lang'])){
			...
			<input type='text' name='l_name'value='".$row['lang_name']."' placeholder='Enter Language name'>
				<center><button name='edit_lang'>Edit Language</button></center>
		</form>";
			
			if(isset($_POST['edit_lang'])){
				$lang_name=$_POST['l_name'];
				$check=$con->prepare("select * from lang where lang_name='$lang_name'");
...
function view_lang()
{
	...
	$i=1;
		...
		<td>".$i++."</td>
		<td>".$row['lang_name']."</td>
		<td><a href='index.php?lang&edit_lang=".$row['lang_id']."title='Edit'><i class='fas fa-edit'></i></a>
		<a style='color:#f00' href='index.php?lang&del_lang=".$row['lang_id']."'title='Delete'><i class='fas fa-trash'></i></a></td>
		</tr>";
	endwhile;
...
function view_cat()
{
	...
	$i=1;
		...
		<td>".$i++."</td>
		<td>".$row['cat_name']."</td>
		<td><a  href='index.php?cat&edit_cat=".$row['cat_id']."'title='Edit'><i class='fas fa-edit'></i></a>
		<a style='color:#f00' href='index.php?cat&del_cat=".$row['cat_id']."'title='Delete'><i class='fas fa-trash'></i></a></td>
		</tr>";
	endwhile;
...
function view_sub_cat()
{
	include("inc/db.php");
	 $get_cat=$con->prepare("select * from sub_cat");
		...
		<td><a  href='index.php?sub_cat&edit_sub_cat=".$row['sub_cat_id']."'title='Edit'><i class='fas fa-edit'></i></a>
		<a style='color:#f00' href='index.php?sub_cat&del_sub_cat=".$row['sub_cat_id']."'title='Delete'><i class='fas fa-trash'></i></a></td>
		<tr>";
	endwhile;
		...
		if($del->execute())
		{
         ...
         function select_cat(){
			include("inc/db.php");
			 ...
			 $get_cat->execute();
			 while($row=$get_cat->fetch()):
			 	echo"<option value='".$row['cat_id']."'>".$row['cat_name']."</opyion>";
			 endwhile;
	}
...
function add_term(){
		...
		if(isset($_POST['add_term'])){
			...
			$term_id=$_POST['for_whome'];
			$check=$con->prepare("select * from term where term='$term_name'");
			$check->setFetchMode(PDO:: FETCH_ASSOC);
			$check->execute();
			$count=$check->rowcount();
			if($count==1){
				...
				$add_term=$con->prepare("insert into term(term,for_whome)values('$term_name','$term_id')");
			if($add_term->execute()){
...
function edit_term(){
		...
		if(isset($_GET['edit_term'])){
			...
			echo "	</select>
			<input type='text' name='term'value='".$row['term']."' placeholder='Enter T&C'>
			...
			if(isset($_POST['edit_t'])){
				...
				$up=$con->prepare("update term set term='$term_name' ,for_whome='$term_id'where t_id='$id'");
			if($up->execute()){
				echo"<script>alert('T&C Updated Successfully')</script>";
				echo"<script>window.open('index.php?terms','_self')</script>";
			}
			else{
				echo"<script>alert('Update Failed')</script>";
...
function view_term()
{
	...
	$i=1;
		...
		<td>".$row['term']."</td>
		<td>".$row['for_whome']."</td>
		<td><a  href='index.php?terms&edit_term=".$row['t_id']."'title='Edit'><i class='fas fa-edit'></i></a>
		<a style='color:#f00' href='index.php?terms&del_term=".$row['t_id']."'title='Delete'><i class='fas fa-trash'></i></a></td>
		<tr>";
		endwhile;
...
function contact(){
	...
	echo"
			<form method='post' enctype='multipart/form-data'>
				...
				<center><button name='up_con'>Add Details</button></center>
		</form>
...
function add_faqs(){
		...
		if(isset($_POST['add_faqs'])){
			...
			if($count==1){
				echo"<script>alert('FAQ's Already Added')</script>";
				echo"<script>window.open('index.php?faqs','_self')</script>";
			}else{
				$add_faqs=$con->prepare("insert into faqs(qsn,ans) values('$qsn','$ans')");
			if($add_faqs->execute()){
				echo"<script>alert('FAQ's Added Successfully')</script>";
				echo"<script>window.open('index.php?faqs','_self')</script>";
			}
			else{
				echo"<script>alert('FAQ's Not Added Successfully')</script>";
				echo"<script>window.open('index.php?faqs','_self')</script>";
			} 
			}
...
function view_faqs(){
	...
	echo"<details>
			...
			<input type='hidden' name='id' value='".$row['q_id']."' />
				...
				<center><button name='up_faqs'>Update FAQ's</button></center>
		</form>
		</details><br />";
	endwhile;
	...
	if(isset($_POST['up_faqs'])){
			...
			if($up_faqs->execute()){
				echo"<script>alert('FAQ's Updated Successfully')</script>";
				echo"<script>window.open('index.php?faqs','_self')</script>";
			}
			else{
				echo"<script>alert('Update Failed')</script>";
